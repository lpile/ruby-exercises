...
class NestedTest < MiniTest::Test
  ...
  def test_list_of_employees_across_all_restaurants
    employee_names = []
    stores.each_value {|value| employee_names.concat(value[:employees])}
  ...
  def test_list_of_all_ingredients_across_all_restaurants
    ingredients = []
    stores.each_value {|value| value[:dishes].each {|dish| ingredients.concat(dish[:ingredients])}}
  ...
  def test_menu_accross_all_restaurants
    full_menu = Hash.new
    stores.values.each do |value|
      value[:dishes].each {|dish| full_menu[dish[:name]] = dish}
    end
